...
type Country struct {
	...
	ActiveTraits           []string           `json:"active_traits"`            // Array of current trait IDs or names
	...
	DiplomaticStances      []DiplomaticStance `json:"diplomatic_stance"`        // Relations to other countries
	Secrets                []string           `json:"secrets"`                  // Array of strings representing hidden information/secrets
	InternalSecurity       int                `json:"internal_security"`        // Range: 0-100
	Status                 []string           `json:"status"`                   // e.g. "Bankrott", "erobert", "Stabil"
